...
const suprisedBtn = document.querySelector('button.suprised');
const sambaBtn = document.querySelector('button.samba');
...
const lookBtn = document.querySelector('button.look');
const skeletalBtn = document.querySelector('button.skeletal');
const modelBtn = document.querySelector('button.model');
  

  
function init() {
    // Link do strony z której został pobrany model https://sketchfab.com/3d-models/geralt-of-rivia-c0c479d36d7d402ea800eddf7b0cac9b?fbclid=IwAR0KNQ0oGSvjuuHMKX7gTWCCmXfKOCPpoLbnKC8pVKtYy6P_CD6jk1rjkJA
    const MODEL_PATH = 'gilbert-anim.glb';
    ...
    // Texture 
    let gilbert_txt = new THREE.TextureLoader().load('texturaBC.jpg');
    gilbert_txt.flipY = true;

    const gilbert_mtl = new THREE.MeshPhongMaterial({
    map: gilbert_txt,
    color: 0xffffff,
    skinning: true });
...
function keyAction(e) {
    if (e.code == 'KeyS') toggleSkeletalVisibility();
    if (e.code == 'KeyM') toggleModelVisibility();
    if (e.code == 'Space') playOnClick('jump');

    if (['Digit1', 'Numpad1'].includes(e.code)) playOnClick('idle');
    if (['Digit2', 'Numpad2'].includes(e.code)) playOnClick('warmup');
    if (['Digit3', 'Numpad3'].includes(e.code)) playOnClick('suprised');
    if (['Digit4', 'Numpad4'].includes(e.code)) playOnClick('samba');
    if (['Digit5', 'Numpad5'].includes(e.code)) playOnClick('dance');
    if (['Digit6', 'Numpad6'].includes(e.code)) playOnClick('waving');
    if (['Digit7', 'Numpad7'].includes(e.code)) playOnClick('punching');
    if (['Digit8', 'Numpad8'].includes(e.code)) playOnClick('kicking');
    if (['Digit9', 'Numpad9'].includes(e.code)) playOnClick('look');
...
modelBtn.addEventListener('click', toggleModelVisibility);
...
suprisedBtn.addEventListener('click', e => playOnClick('suprised'));
sambaBtn.addEventListener('click', e => playOnClick('samba'));
...
lookBtn.addEventListener('click', e => playOnClick('look'));
